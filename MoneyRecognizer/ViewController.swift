...
class ViewController: UIViewController {
    ...
    lazy var classificationRequest: [VNRequest] = {
        do {
            ...
            let model = try VNCoreMLModel(for: moneyrecognizer().model)
    ...
    func handleClassification(request: VNRequest, error: Error?) {
        ...
        if best.identifier.starts(with: "Unknown") || best.confidence < confidence {
            ...
            DispatchQueue.main.async {
                ...
                let predictedClass = best.identifier.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines)
                self.bubbleLayer.string = predictedClass
                
                self.textToSpeech(text: predictedClass)
            }
        }
    }
